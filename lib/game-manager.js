...
Game.prototype.move = function(){
  ...
  this.playerA.client.send({ type: "move", move: this._currentMove, points: this.points.playerA });
  !this.single && this.playerB.client.send({ type: "move", move: this._currentMove, points: this.points.playerB });
};
...
Game.prototype.handleMove = function(player, countryName){
  ...
  if(countryName !== "pass"){
    ...
    this.points[player] += this._currentMove.commodity.price * country.multiplier * (caught ? -1 : 1);
  }
...
Game.prototype.finish = function(){
  if(this.points.playerA <= 0){
    this.playerA.client.send({ type: "fml", bankrupt: true });
    !this.single && this.playerB.client.send({ type: "ftw", bankrupt: true });
  }else if(this.single){
    this.playerA.client.send({ type: "ftw", single: true });
  }else if(this.points.playerB <= 0){
    this.playerA.client.send({ type: "ftw", bankrupt: true });
    this.playerB.client.send({ type: "fml", bankrupt: true });
  }else{
    var winner = this.points.playerA == this.points.playerB ? "tie" : this.points.playerA > this.points.playerB ? "playerA" : "playerB";
    if(winner === "tie"){
      this.playerA.client.send({ type: "ftw", tie: true });
      this.playerB.client.send({ type: "ftw", tie: true });
    }else if(winner === "playerA"){
      this.playerA.client.send({ type: "ftw", points: true });
      this.playerB.client.send({ type: "fml", points: true });
    }else if(winner === "playerB"){
      this.playerA.client.send({ type: "fml", points: true });
      this.playerB.client.send({ type: "ftw", points: true });
    }
  }
};